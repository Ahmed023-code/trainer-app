...
export async function getUSDADatabase(): Promise<Database> {
  ...
  initPromise = (async () => {
    try {
      ...
      // Set NEXT_PUBLIC_USDA_DB_URL environment variable (e.g., "https://your-cdn.com/usda-test.sqlite")
      // Using test DB for now - full 2.1GB DB too large for browser
      console.log('[USDA DB] Fetching test database (100 foods)...')

      const dbUrl = process.env.NEXT_PUBLIC_USDA_DB_URL || '/usda-test.sqlite'
      const response = await fetch(dbUrl)
      if (!response.ok) {
        throw new Error(
          `Failed to load database from ${dbUrl}. Please configure NEXT_PUBLIC_USDA_DB_URL environment variable or ensure the database file is available at the expected path.`,
        )
      }
      ...
      console.log(
        '[USDA DB] Database contains',
        testResult[0].values[0][0],
        'foods',
      )
