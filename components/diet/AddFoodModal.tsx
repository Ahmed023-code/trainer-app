...
// Alias of QuickAddModal for older imports that expect AddFoodModal.
// Keeps API compatible and avoids circular imports.
  ...
  return (
    <>
      ...
      <div className="fixed bottom-24 right-6 z-[9999] w-[18rem] rounded-2xl border border-neutral-200 dark:border-neutral-800 bg-white/95 dark:bg-neutral-900/95 shadow-2xl backdrop-blur p-3 space-y-2">
        <div className="text-lg font-semibold">Add Food — {meal}</div>

        <label className="block text-sm">Name
          <input className="mt-1 w-full rounded-lg border px-3 py-2 bg-white dark:bg-neutral-900"
            value={form.name} onChange={(e)=>setForm(f=>({...f, name: e.target.value}))} placeholder="Food name" />
        </label>
        ...
        <div className="pt-1 flex justify-end gap-2">
          ...
          <button className="px-3 py-2 rounded-lg bg-[#34D399] text-black disabled:opacity-50"
            disabled={!canSave}
            onClick={()=>onSave({
