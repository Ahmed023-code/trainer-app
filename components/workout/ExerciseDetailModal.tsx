...
export default function ExerciseDetailModal({
  ...
  const isQuickAdd =
    !localExercise.source || localExercise.source === 'quick-add'
  ...
  const deleteSet = (idx: number) => {
    const next = {
      ...localExercise,
      sets: localExercise.sets.filter((_, i) => i !== idx),
    }
    setLocalExercise(next)
  }
      ...
      <div className="absolute inset-0 bg-white dark:bg-neutral-900 flex flex-col">
        ...
        <div className="sticky top-0 z-10 p-4 px-5 bg-white/90 dark:bg-neutral-900/90 backdrop-blur border-b border-neutral-200 dark:border-neutral-800">
          ...
          <div>
            ...
            {/* Target Muscles */}
            {Array.isArray((localExercise as any).targetMuscles) &&
              (localExercise as any).targetMuscles.length > 0 && (
                <div className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
                  <span className="font-medium">Target:</span>{' '}
                  {(localExercise as any).targetMuscles
                    .map((m: string) => m.charAt(0).toUpperCase() + m.slice(1))
                    .join(', ')}
                </div>
              )}
            {/* Secondary Muscles */}
            {Array.isArray((localExercise as any).secondaryMuscles) &&
              (localExercise as any).secondaryMuscles.length > 0 && (
                <div className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
                  <span className="font-medium">Secondary:</span>{' '}
                  {(localExercise as any).secondaryMuscles
                    .map((m: string) => m.charAt(0).toUpperCase() + m.slice(1))
                    .join(', ')}
                </div>
              )}
            <div className="text-xs text-neutral-500 dark:text-neutral-400 mt-0.5">
              {localExercise.sets.length} set
              {localExercise.sets.length === 1 ? '' : 's'}
            </div>
          </div>
                    ...
                    {isQuickAdd ? (
                      ...
                      <>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateSetField(i, 'repsPerformed', e.target.value)
                          }
        ...
        <div
          className="sticky bottom-0 z-10 p-4 bg-white dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-800"
