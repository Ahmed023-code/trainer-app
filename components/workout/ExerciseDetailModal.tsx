    ...
    <div className="fixed inset-0 z-[9600]">
      ...
      <div className="absolute inset-0 bg-white dark:bg-neutral-900 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-1 space-y-4">
          ...
          {/* Column headers */}
          <div className="grid grid-cols-[24px,90px,70px,1fr,40px,36px] gap-1 px-1 text-xs font-semibold text-neutral-600 dark:text-neutral-400 uppercase tracking-wide">
          ...
          <div className="space-y-2">
            {localExercise.sets.map((s, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`grid grid-cols-[24px,90px,70px,1fr,40px,36px] gap-1 items-center rounded-full border px-1 py-2.5 ${getSetColor(s.type)}`}
                >
                  {/* Set number */}
